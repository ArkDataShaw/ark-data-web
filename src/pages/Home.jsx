  ...
  return (
    <div ref={ref} style={{ background: '#06162A', border: '1px solid #0A2142', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 0 40px rgba(10,33,66,0.6)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
        {[
          { label: 'Total Sessions', val: fmt(sessions), delta: '+12%', color: S.muted },
          { label: 'Identified', val: fmt(identified), delta: '+15%', color: '#DFFFEF' },
          { label: 'High Intent', val: fmt(highIntent), delta: '+8%', color: S.muted },
          { label: 'Conversions', val: fmt(conversions), delta: '+5%', color: '#DFFFEF' },
        ].map((s, i) => (
      ...
      <div style={{ padding: '16px 20px', borderBottom: '1px solid #0A2142' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', height: '40px' }}>
          {[60,72,58,80,90,75,88,94,70,85,92,78,88,96,82,90,88,95,80,92,98,84,90,96].map((h, i) => (
            <div key={i} style={{ flex: 1, background: i > 18 ? '#1a5ca8' : '#063524', height: `${h}%`, borderRadius: '2px 2px 0 0', minWidth: '4px', transition: 'height 0.3s' }} />
          ))}
        </div>
      </div>
...
const features = [
  { title: 'Real-Time Enrichment', desc: 'Enrich visitor sessions and CRM records with firmographic, technographic, and contact data as signals emerge.', border: S.blueMid, bg: S.blue },
  { title: 'Intent Signals & Scoring', desc: 'AI-scored intent across 10,000+ data points. Know who is in-market before your competitors do.', border: S.blueMid, bg: S.blue },
  { title: 'Firmographic + Technographic', desc: 'Industry, headcount, revenue band, tech stack, HQ location, decision-maker roles—all enriched automatically.', border: S.greenMid, bg: S.green },
  { title: 'CRM & Platform Delivery', desc: 'Push enriched, scored data directly to Salesforce, HubSpot, Klaviyo, ad platforms, or custom webhooks.', border: S.greenMid, bg: S.green },
  { title: 'Data Quality Controls', desc: 'Validation, deduplication, bounce suppression, and normalization on every record before delivery.', border: S.blueMid, bg: S.blue },
  { title: 'Compliance & Governance', desc: 'Designed to support GDPR, CCPA, and enterprise data governance requirements. DPA available.', border: S.greenMid, bg: S.green },
  { title: 'Custom Segments', desc: 'Build audience segments by industry, intent category, tech stack, or any combination of attributes.', border: S.blueMid, bg: S.blue },
  { title: 'API + Batch Delivery', desc: 'Flexible delivery: real-time API, batch CSV export, or direct CRM/ESP integration.', border: S.greenMid, bg: S.green },
  { title: 'Deduplication + Normalization', desc: 'Every record cleaned, standardized, and deduplicated before it hits your stack.', border: S.blueMid, bg: S.blue },
];
  ...
  return (
    <div style={{ background: '#000002', color: '#fff' }}>
      ...
      <section style={{ position: 'relative', minHeight: '92vh', display: 'flex', alignItems: 'center', overflow: 'hidden', background: 'linear-gradient(135deg, #000002 0%, #020D1F 50%, #000D08 100%)' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '28px' }}>
                ...
                <Link to={createPageUrl('Pricing')}>
                  <button className="ark-btn-blue" style={{ padding: '14px 28px', fontSize: '16px' }}>
                    View Pricing
                  </button>
                </Link>
              </div>
              ...
              <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', paddingTop: '20px', borderTop: '1px solid #0A2142' }}>
                <div>
                  <p style={{ color: '#fff', fontWeight: 800, fontSize: '22px' }}>10M+</p>
                  <p style={{ color: S.muted, fontSize: '12px' }}>Records enriched/mo</p>
                </div>
                <div>
                  <p style={{ color: '#fff', fontWeight: 800, fontSize: '22px' }}>95%</p>
                  <p style={{ color: S.muted, fontSize: '12px' }}>Match accuracy</p>
                </div>
                <div>
                  <p style={{ color: '#fff', fontWeight: 800, fontSize: '22px' }}>&lt;15min</p>
                  <p style={{ color: S.muted, fontSize: '12px' }}>Pixel install</p>
                </div>
              </div>
            </div>
        ...
        <div style={{ position: 'absolute', bottom: '24px', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: '24px', zIndex: 2 }}>
          {[['How It Works', 'HowItWorks'], ['Pricing', 'Pricing'], ['Security', 'Security']].map(([label, page]) => (
            <Link key={page} to={createPageUrl(page)} className="ark-link" style={{ color: S.muted, fontSize: '12px', fontWeight: 500, letterSpacing: '0.05em' }}>
              {label} →
            </Link>
          ))}
        </div>
      ...
      <section style={{ background: '#06162A', borderTop: '1px solid #0A2142', borderBottom: '1px solid #0A2142', padding: '24px 0' }}>
        <div className="sc">
          <p style={{ color: S.muted, fontSize: '12px', fontWeight: 600, textAlign: 'center', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '20px' }}>
      ...
      <section className="sp" style={{ background: '#000002' }}>
        <div className="sc">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            {painPoints.map((p, i) => (
              <div key={i} className="ark-card" style={{ padding: '28px', transition: 'border-color 0.2s' }}>
        ...
        <div className="sc">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
            ...
            <div style={{ background: '#06162A', border: '1px solid #0A2142', borderRadius: '12px', padding: '40px', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ color: S.muted, fontSize: '14px', lineHeight: 1.7, marginBottom: '24px' }}>
                Convert anonymous website visitors into verified identities with company data, contact info, and behavioral signals. Route them to your CRM before they go dark.
              </p>
              ...
              <Link to={createPageUrl('Services')}>
                <button className="ark-btn-blue" style={{ padding: '10px 20px', fontSize: '13px' }}>Learn More →</button>
              </Link>
            </div>
            ...
            <div style={{ background: '#042016', border: '1px solid #063524', borderRadius: '12px', padding: '40px', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ color: '#DFFFEF', fontSize: '14px', lineHeight: 1.7, marginBottom: '24px' }}>
                Access verified individuals actively researching your solution category across the web. Know who is in-market today, what they're evaluating, and reach them first.
              </p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '28px' }}>
              ...
              <Link to={createPageUrl('Services')}>
                <button className="ark-btn-green" style={{ padding: '10px 20px', fontSize: '13px' }}>Learn More →</button>
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="sp" style={{ background: '#000002' }}>
        <div className="sc">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
            {features.map((f, i) => (
              <div key={i} className={i % 2 === 0 ? 'ark-card' : 'ark-card-green'} style={{ padding: '28px', transition: 'all 0.2s' }}>
      ...
      <section className="sp" style={{ background: '#000002' }}>
        <div className="sc">
          <div style={{ textAlign: 'center', marginBottom: '56px' }}>
            ...
            <p style={{ color: S.muted, fontSize: '16px', maxWidth: '480px', margin: '0 auto' }}>Four steps from pixel install to your first qualified lead. No complex setup required.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '2px', position: 'relative' }}>
            {[
              ...
              <div key={i} style={{ background: '#06162A', border: '1px solid #0A2142', borderRadius: '10px', padding: '32px', position: 'relative' }}>
      ...
      {/* ── TRUST / COMPLIANCE ── */}
      <section style={{ background: '#020D1F', borderTop: '1px solid #0A2142', padding: '60px 0' }}>
        <div className="sc">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '60px', alignItems: 'center' }}>
            <div>
              <p style={{ color: '#DFFFEF', fontSize: '12px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '14px' }}>Privacy-First</p>
              <h2 style={{ fontSize: 'clamp(24px, 2.5vw, 36px)', fontWeight: 900, letterSpacing: '-0.8px', marginBottom: '16px' }}>Compliance-Grade Data You Can Trust.</h2>
              <p style={{ color: S.muted, fontSize: '15px', lineHeight: 1.7, marginBottom: '28px' }}>
                Ark Data is designed to support GDPR, CCPA, and enterprise governance requirements. Every record handled with precision and accountability.
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {['Encryption in transit', 'Role-based access', 'DPA available', 'Vendor review ready', 'Data residency options', 'Audit trail logging'].map(badge => (
                  <div key={badge} style={{ background: '#042016', border: '1px solid #063524', borderRadius: '6px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <CheckCircle size={14} style={{ color: '#22c55e', flexShrink: 0 }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {[['95%', 'Match Accuracy', 'Industry-leading precision with verified B2B data'], ['10M+', 'Records Processed/mo', 'Scaled infrastructure with real-time enrichment'], ['<1 wk', 'Average Go-Live', 'From contract to first lead in under a week']].map(([n, l, d]) => (
                <div key={l} style={{ background: '#06162A', border: '1px solid #0A2142', borderRadius: '8px', padding: '20px 24px', display: 'flex', gap: '20px', alignItems: 'center' }}>
                  <p style={{ color: '#B1001A', fontWeight: 900, fontSize: '28px', letterSpacing: '-1px', flexShrink: 0, minWidth: '70px' }}>{n}</p>
                  <div>
                    <p style={{ color: '#fff', fontWeight: 700, fontSize: '14px', marginBottom: '4px' }}>{l}</p>
                    <p style={{ color: S.muted, fontSize: '12px' }}>{d}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section style={{ background: '#000002', borderTop: '1px solid #0A2142', padding: '60px 0' }}>
        <div className="sc" style={{ textAlign: 'center' }}>
          ...
          <Link to={createPageUrl('Integrations')}>
            <button className="ark-btn-blue" style={{ padding: '10px 24px', fontSize: '13px' }}>View all integrations →</button>
          </Link>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '32px' }}>
            {[
              ...
              <div key={i} style={{ background: plan.highlight ? '#06162A' : '#000002', border: `1px solid ${plan.highlight ? '#1a5ca8' : '#0A2142'}`, borderRadius: '10px', padding: '32px', position: 'relative', boxShadow: plan.highlight ? '0 0 24px rgba(26,92,168,0.2)' : 'none' }}>
                ...
                <Link to={createPageUrl('BookADemo')}>
                  <button className={plan.highlight ? 'ark-btn-red' : 'ark-btn-blue'} style={{ width: '100%', padding: '12px', fontSize: '14px' }}>Get Started</button>
                </Link>
              </div>
            ))}
          </div>
      ...
      <section style={{ background: 'linear-gradient(135deg, #06162A 0%, #000002 40%, #042016 100%)', borderTop: '1px solid #0A2142', padding: '80px 0' }}>
        <div className="sc" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: S.muted, fontSize: '18px', maxWidth: '480px', margin: '0 auto 36px', lineHeight: 1.65 }}>
            Get a personalized walkthrough and see how Ark Data fits into your stack.
          </p>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link to={createPageUrl('BookADemo')}>
              <button className="ark-btn-red" style={{ padding: '16px 36px', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                Book a Call <ArrowRight size={16} />
              </button>
            </Link>
            <Link to={createPageUrl('Pricing')}>
              <button className="ark-btn-green" style={{ padding: '16px 36px', fontSize: '16px' }}>
