...
// ── Pricing formula (locked 2026-05-07) ────────────────────────────────────
// Free through 200 EV, then 6.5 × √(EV − 200) up to 12,500 EV.
// Above 12,500 EV the marketing site stops self-serve and routes to Contact
// Sales — the calculator hides the dollar amount past that threshold.
// Mirror of src/lib/billingPricing.js.
const FREE_THRESHOLD = 200;
const SQRT_COEFFICIENT = 6.5;
const CONTACT_THRESHOLD = 12500;

function calculateCost(enrichedVisits) {
  if (enrichedVisits <= FREE_THRESHOLD) return 0;
  return SQRT_COEFFICIENT * Math.sqrt(enrichedVisits - FREE_THRESHOLD);
}
      ...
      <div style={{ background: 'linear-gradient(145deg, #071829 0%, #040E1A 100%)', border: '1px solid rgba(26,92,168,0.5)', borderRadius: '16px', padding: '36px', marginBottom: '28px', boxShadow: '0 0 60px rgba(26,92,168,0.1)' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(10,33,66,0.8) 0%, rgba(4,14,26,0.9) 100%)', border: '1px solid rgba(26,92,168,0.5)', borderRadius: '12px', padding: '24px 28px', marginBottom: '20px' }}>
          <p style={{ color: '#7eb8ff', fontSize: '11px', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '8px' }}>
            {enrichedVisits > CONTACT_THRESHOLD ? 'Enterprise Volume' : 'Your Estimated Monthly Cost'}
          </p>
          {enrichedVisits > CONTACT_THRESHOLD ? (
            <div>
              ...
              <p style={{ color: '#D9ECFF', fontSize: '13px', lineHeight: 1.6, marginBottom: '18px', maxWidth: '520px' }}>
                For volumes above {fmt(CONTACT_THRESHOLD)} enriched visits per month, contact our team for custom pricing including annual prepay, SLA, and dedicated support.
              </p>
        ...
        {/* Pricing details */}
        {enrichedVisits > FREE_THRESHOLD && enrichedVisits <= CONTACT_THRESHOLD && (
        ...
        {enrichedVisits > FREE_THRESHOLD && enrichedVisits <= CONTACT_THRESHOLD && (rb2b || opensend) && (
        ...
        {/* CTA (hidden above the Contact Sales threshold) */}
        {enrichedVisits <= CONTACT_THRESHOLD && (
          <>
