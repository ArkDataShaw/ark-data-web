...
- **Filters-popover parity pass (2026-07-18, mockup `3cb06ff`→`bb640e2`)**: seniority is now the app's 2-level hierarchy (parents derived, children = the old flat enum → buildSpec/resolver untouched); collapsed-picker "Match" border leak fixed (vertical padding only on `.sub.open` — a 0fr grid track can't shrink below child padding); ZIP radius UI rebuilt to app parity (Manual|Radius toggle, 1–100mi slider, gated Search, expandable "N zips in Xmi" disclosure with removable chips + Clear). **Radius is now the DEFAULT Zip sub-mode on all four interactive surfaces** — mockup `bb640e2`, app `b64453a`, react standalone `781b57b` (manual only pre-selects when hand-entered ZIPs exist without a radius). The vendored #4 copy has no radius mode at all (re-vendor HOLD).
- **Resolver parity fix (2026-07-19, `4bad5b7`)**: the shared fn's `department`→`department_2` mis-mapping and missing seniority-hierarchy resolution fixed — insightsAgg (and every action) now emits app-identical filter payloads. Any future light/heavy work inherits correct spec→API mapping.
