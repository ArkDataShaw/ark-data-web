...
**2026-07-19 — vendored copy caught up + shared-resolver bugs fixed:**
1. **The #4 vendored builder is NO LONGER the laggard.** Five [CORE] commits ported (agreed with + reviewed/pushed by arkdata-web-opus-5, CI-deployed): tri-excludes `660bcf2`, seniority hierarchy + border-leak `30ea67e`, radius UI `64cdc49`, radius default `be07aed`, and a port-verification finding — zip-lookup.js was width-gated >1000px so radius would have silently no-oped on mobile/tablet; now lazy-loaded on Zip-tab open (`2618e47`). Deploy model changed: ark-data-web is now git-connected CI (push = deploy); the demo-layer guardrails (ark-fullheight/__arkScripted/strip CSS) were untouched and opus-5 ran its 7-item beats/reversibility checklist before pushing.
2. **Shared demo resolver bugs (mockup fn — serves mockup + vendored `/api/audience` proxy + proposals), fixed in `4bad5b7`:** (a) `department` mapped to `department_2` — the wrong upstream enum, fixed in react `038e095` + the app long ago but drifted here; (b) no hierarchy resolution — granular-always meant a fully-checked C-Suite returned a different audience than the app (P2/P4). Now app-parity: full groups → broad `seniority_level`, partial → granular `_2` (never both — API ANDs), CXO-only → both for precision. Unit-harnessed (5 cases match the app's filter shapes) + live-verified (dept 13,960 / full C-Suite 56,203 / CXO-only 17,383).
2b. **`.cbrow` alignment fix (2026-07-19/20):** every 1st-level checkbox row in the filters popover sat 10px right of the app's position — the app's `.cbrow` carries `margin-left:-10px` (audience.css:872, hover-pill padding compensation) which the HTML port predated. 2nd-tier hierarchy rows were already correct (`.hier-child`'s 8px margin is a non-additive override). Fixed on mockup `85602f3` (→ proposals + standalone) and vendored `0e48df5` (opus-5 reviewed/pushed, CI-deployed, verified live on arkdata.io). Measured post-fix: 1st-level inputs 23px from sidebar edge, children 41px, 18px indent = app geometry.
3. **Net state: all five surfaces at parity on every audited dimension.** Remaining deltas are intentional (app whitelabel colors/flat donuts/segment-locks) or trivial nits (#2 StatRow tooltip, `void pc`). The re-vendor question is MOOT — the vendored copy was brought up by targeted port instead.
...
### 7.4 Single-open accordion rollout (started 2026-08-03)

Shaw's directive: every filter menu in the family adopts the custom-intent single-open accordion (spec: `audience-builder-react/docs/PATTERN-accordion-single-open.md`, a108b1e — open-state is ONE nullable slot per level, `slot = slot===id ? null : id`; sub slot resets on parent switch).

**Step 1 SHIPPED — #2 react standalone standard sidebar, by arkdata-app-opus-8** (`87d9b41`, deployed): 2 files +16/−10 — the canonical six edit sites: type (store:47-48), init (:81-82), toggles (:239-240, toggleAcc also nulls openSub), setMode 'topics' auto-open append→slot-write (:180), 2 render consumers `.includes`→`===` (Sidebar:88,118). `restore()` untouched (already shape-agnostic). Full report: `/tmp/opus8-accordion-standard-report.md` (10/10 checklist, desktop+mobile, 59 tests pass).

**Remaining surfaces (planned, NOT executed):** in order — #3 app (`apps/web` store.tsx:38-39/:128/:134-135/:224 + 2 Sidebar consumers, byte-identical shape → mechanical twin-port) → #1 mockup vanilla (S.openAcc/openSub Sets :1191; click handlers :1432-34/:1574; custom auto-open :2029; keep the Set CONTAINER, single-membership enforced in USER click handlers only) → #4 vendored (same patch :1180/:1239-40/:1380/:1844; **constraint: demo layer embed-script.js:464 stages beat 1 with TWO open cats `['topics','personal']` and replaces the Sets wholesale — scripted staging must stay multi-capable, hence click-handler-only enforcement; full scroll-demo regression before push; push owner TBD, opus-5 gone**) → #7 proposals-refactor (identical zustand shape store.tsx:45-46/:154/:176-177/:266 + Sidebar:78,108; their lineage applies, we hand over the report). #6 custom = reference; #5 has no filter sidebar; mobile satellites dormant/dead. Key gotchas for ports: hunt appends outside the toggle (mode-switch/auto-open), reset sub slot on parent change, `>` child combinator, localStorage shape migration if a surface persists open-state (none of ours do).

### 7.5 Single-open rollout — 2026-08-03 code sweep + per-surface implementation plan

Full-codebase sweep of every remaining surface (read-only). Residual check on #2 at `87d9b41`: CLEAN — no missed consumers, no other multi-open accordions (hierarchy picker is stateless; location tabs/radiusExpanded are component-local, out of scope); standard-vs-custom implementations now behaviorally identical (both reset sub on parent switch).

**#3 app.arkdata.io — 9 edit sites, zero surprises.** store.tsx :38-39 type, :60-61 init, :128 setMode append→slot write (+openSub reset), :134-135 toggles (toggleAcc also nulls openSub), Sidebar.tsx :70/:100 `.includes`→`===`. restore() :224 unchanged. CSS already correct (audience.css:800/:804 grid-rows, collapsed override :702). Open-state never persisted/serialized (buildSpec/specToState skip it); no tests; sub-ids globally unique; segment-locks already stripped on the feat branch; diff applies identically to main (no theming divergence in these regions).

**#1 mockup — 3 clear() insertions, animation-safe.** Cat click :1433-34 (`S.openAcc.clear()` before add + `S.openSub.clear()` on parent switch), sub click :1574, setMode :2029 (clear both + add 'topics' — also fixes a pre-existing stale-sub bug). renderSidebar's reflow+rAF delta batching (:1416-1447) animates simultaneous close+open correctly (both deltas land in one frame; `>` combinator :89 isolates bodies). Embed mode shares the same state untouched. Open question: clearAllFn :2043 leaves the accordion open after Clear All (likely keep as-is).

**#4 vendored — same patch, demo constraint confirmed and sharpened.** Regions byte-identical to the mockup (line map: init 1191→1180, cat click 1432-36→1238-42, sub click 1574→1380, setMode 2029→1844). embed-script.js:487-489/:585 REPLACES the Sets wholesale per beat; beat 1 stages TWO open cats `['topics','personal']`; the :487 idempotency check is `sameSet()`. Verdict: click-handler-only enforcement is sound — scripted staging keeps rendering multi-member Sets untouched. Residual risk: a user click DURING scripted playback would collapse the staged pair and the next beat re-stage re-expands it (flicker) — same class of interruption that exists today; verify the demo's interaction guards during regression, change nothing in embed-script.js.
...
**§7.6 update (2026-08-03, Shaw):** #4 vendored single-open REVERTED — `69ef983` reverts `cf46e62`, CI-deployed, confirmed live (arkdata.io builder back to multi-open). The other four surfaces (react `87d9b41`, app `1119593`, mockup `ff7dd44`, proposals-refactor `338add2`) remain single-open. #4 is now the intentional outlier pending Shaw's direction.
